...
interface HonorCardProps {
  ...
  unlocked: boolean;
  isFlipped?: boolean; // Controlled state for flip
  onClick?: () => void;
}

export const HonorCard: React.FC<HonorCardProps> = ({ card, unlocked, isFlipped = false, onClick }) => {
  return (
    <div 
      className="group perspective-[1000px] w-full aspect-[3/5] cursor-pointer"
      onClick={() => unlocked && onClick && onClick()}
    >
      <div className={`relative w-full h-full duration-700 [transform-style:preserve-3d] transition-all ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
        ...
        <div className={`absolute inset-0 [backface-visibility:hidden] rounded-2xl border-4 shadow-xl overflow-hidden flex flex-col items-center p-2 ${unlocked ? 'bg-white border-white' : 'bg-gray-200 border-gray-300 opacity-90'}`}>
          ...
          <h3 className={`font-black text-xs md:text-sm mb-1 leading-tight text-center ${unlocked ? 'text-gray-800' : 'text-gray-500'}`}>{card.title}</h3>
          ...
          <div className="flex-1 flex items-center justify-center w-full px-1">
             ...
             {unlocked && <span className="text-[10px] text-sky-400 font-bold opacity-0 group-hover:opacity-100 transition-opacity">✨ 点击查看</span>}
          </div>
        </div>
